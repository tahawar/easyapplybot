...
class EasyApplyBot:
    ...
    def fill_out_fields(self):
        # Handle text inputs for experience questions
        # experience_questions = self.browser.find_elements(By.XPATH, "//input[contains(@id, 'numeric')]")
        # for question in experience_questions:
        #     question.clear()
        #     # You might want to customize the number of years based on the question if needed
        #     question.send_keys("5")  # Example: 5 years of experience

        # # Handle dropdowns for proficiency or other similar questions
        # proficiency_dropdowns = self.browser.find_elements(By.XPATH, "//select[contains(@id, 'multipleChoice')]")
        # for dropdown in proficiency_dropdowns:
        #     select = Select(dropdown)
        #     # Select 'Professional' as an example, change as needed
        ...
        # Handling select dropdowns for 'Yes' and 'No' options
    ...
    # Find all question fields
        questions = self.browser.find_elements(By.XPATH, "//input[@type='text'] | //input[@type='radio']")
        for question in questions:
            label = question.find_element(By.XPATH, "./preceding-sibling::label").text.lower()
            if 'years of work experience' in label:
                # Input a random number of years between 1 and 10 for work experience fields
                question.send_keys(str(random.randint(1, 10)))
            elif 'sponsorship for employment visa status' in label:
                # Handle radio button for visa sponsorship
                choice = 'no'  # or 'yes' depending on user's preference
                if choice in question.get_attribute('value').lower():
                    self.browser.execute_script("arguments[0].click();", question)

    def ans_question(self, question):
        # Simplified answer mapping logic for demonstration
        mappings = {
            'work experience': '5',  # Default experience years
            'sponsorship for employment visa status': 'no'
        }
        for key, value in mappings.items():
            if key in question:
                return value
        return None  # Return None if no suitable answer is found
